...
# ============================================================
# note シェア用の選手カード画像生成 (1200×630 PNG)
# ============================================================
...
def _load_font(size: int, bold: bool = False):
    for p in _CJK_FONT_CANDIDATES:
        try:
            idx = 1 if (bold and p.lower().endswith(".ttc")) else 0
...
@st.cache_data(ttl=60 * 60, show_spinner=False)
def fetch_player_season_stats_for_card(player_id: int, season: int, is_pitcher: bool) -> dict:
    """MLB Stats API から当該選手の season stats を取得。"""
    group = "pitching" if is_pitcher else "hitting"
    url = f"https://statsapi.mlb.com/api/v1/people/{player_id}/stats"
    try:
        r = requests.get(url, params={"stats": "season", "season": season, "group": group}, timeout=10)
        if r.status_code != 200:
            return {}
...
def _headshot_hires_url(player_id: int) -> str:
    return (
        "https://img.mlbstatic.com/mlb-photos/image/upload/"
        "d_people:generic:headshot:67:current.png/w_480,q_auto:best/"
        f"v1/people/{player_id}/headshot/67/current"
    )


def generate_share_card_png(
    player_name_en: str, player_name_ja: str, team: str, position: str,
    season: int, is_pitcher: bool, player_id: int,
) -> bytes:
    """noteシェア用の 1200×630 PNG を生成して bytes で返す。"""
    stat = fetch_player_season_stats_for_card(player_id, season, is_pitcher)
    if is_pitcher:
        metrics = [
            ("ERA",  str(stat.get("era", "—"))),
            ("K",    str(stat.get("strikeOuts", "—"))),
            ("WHIP", str(stat.get("whip", "—"))),
            ("IP",   str(stat.get("inningsPitched", "—"))),
        ]
    else:
        metrics = [
            ("AVG", str(stat.get("avg", "—"))),
            ("HR",  str(stat.get("homeRuns", "—"))),
            ("RBI", str(stat.get("rbi", "—"))),
            ("OPS", str(stat.get("ops", "—"))),
        ]

    img = Image.new("RGB", (_CARD_W, _CARD_H), (13, 17, 23))
    draw = ImageDraw.Draw(img)
    # Top accent bar
    draw.rectangle([(0, 0), (_CARD_W, 10)], fill=(255, 75, 75))

    # Headshot circle
    head_bytes = _fetch_image_bytes(_headshot_hires_url(player_id))
    if head_bytes:
        try:
            h = Image.open(BytesIO(head_bytes)).convert("RGBA").resize((340, 340))
            mask = Image.new("L", (340, 340), 0)
            ImageDraw.Draw(mask).ellipse((0, 0, 340, 340), fill=255)
            img.paste(h, (70, 150), mask)
            draw.ellipse((70, 150, 410, 490), outline=(48, 54, 61), width=4)
        except Exception:
            pass
    ...
    f_sm  = _load_font(28)
    f_xs  = _load_font(22)
    f_sv  = _load_font(56, bold=True)
    f_sl  = _load_font(22)

    x = 460
    # Role chip
    role = "投手" if is_pitcher else "打者"
    chip_c = (230, 57, 70) if is_pitcher else (42, 157, 143)
    draw.rectangle([(x, 120), (x + 96, 162)], fill=chip_c)
    draw.text((x + 14, 124), role, font=f_sm, fill=(255, 255, 255))
    # Meta line
    meta = "  ·  ".join(s for s in [team, position, f"{season}"] if s)
    draw.text((x + 110, 124), meta, font=f_sm, fill=(200, 200, 200))

    # Name (EN)
    draw.text((x, 180), player_name_en, font=f_big, fill=(240, 240, 240))
    # Name (JA)
    if player_name_ja:
        draw.text((x, 272), player_name_ja, font=f_med, fill=(170, 200, 240))
    ...
    for i, (label, val) in enumerate(metrics):
        cx = x + i * (cell_w + 10)
        draw.rectangle([(cx, y0), (cx + cell_w, y0 + 150)],
                       fill=(22, 27, 34), outline=(48, 54, 61), width=2)
        draw.text((cx + 14, y0 + 24), str(val), font=f_sv, fill=(255, 255, 255))
        draw.text((cx + 14, y0 + 112), label, font=f_sl, fill=(139, 148, 158))

    # Footer
    draw.text((70, _CARD_H - 60), "mlb-player-viewer.streamlit.app",
              font=f_xs, fill=(139, 148, 158))
    if NOTE_USER:
        tail = f"note.com/{NOTE_USER}"
        draw.text((_CARD_W - 70 - 10 * len(tail), _CARD_H - 60),
                  tail, font=f_xs, fill=(139, 148, 158))
...
# --- note シェア用カード画像 生成 & ダウンロード ---
with st.expander("🖼️ note 記事用のシェア画像を作成", expanded=False):
    st.caption(
        "1200×630px の PNG を生成します。note のアイキャッチや SNS シェア画像としてご利用ください。"
    )
    if st.button("画像を生成", key=f"gen_share_card_{player_id}"):
        try:
            _jp_raw = JP_ALIASES.get(player_name.lower(), "")
            _jp = _jp_raw.split("/")[0].strip() if _jp_raw else ""
            _team = (profile or {}).get("team") or ""
            _pos  = (profile or {}).get("position") or ""
            with st.spinner("画像を生成中..."):
                png_bytes = generate_share_card_png(
            ...
            st.image(png_bytes, caption="プレビュー", use_container_width=True)
            st.download_button(
                "⬇️ PNG をダウンロード",
                data=png_bytes,
                file_name=f"{player_name.replace(' ', '_')}_{season}_card.png",
                mime="image/png",
                key=f"dl_share_card_{player_id}",
            )
        except Exception as e:
